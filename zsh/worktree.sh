...
(( $+functions[_green] ))  || _green()  { printf '\033[32m%s\033[0m' "$*"; }
(( $+functions[_red] ))    || _red()    { printf '\033[31m%s\033[0m' "$*"; }
(( $+functions[_yellow] )) || _yellow() { printf '\033[33m%s\033[0m' "$*"; }
(( $+functions[_cyan] ))   || _cyan()   { printf '\033[36m%s\033[0m' "$*"; }
(( $+functions[_blue] ))   || _blue()   { printf '\033[34m%s\033[0m' "$*"; }
...
_WT_INFO=$'\u2139\ufe0f'
_WT_WARNING=$'\u26a0\ufe0f'
_WT_SEARCH=$'\U0001f50d'

# --- Configuration ---
_WT_DEFAULT_BRANCH_TYPE="feature"

_WT_VERSION="1.3.0"
_WT_SCRIPT_DIR="${${(%):-%x}:A:h}"
_WT_REPO_DIR="${_WT_SCRIPT_DIR:h}"
...
# Output: wt_path\tbranch\tname\tis_bare (one line per worktree)
_wt_get_parsed_worktrees() {
    ...
    local wt_path="" branch="" is_bare="false"

    while IFS= read -r line; do
        if [[ "$line" == worktree\ * ]]; then
            # Emit previous record if exists
            if [[ -n "$wt_path" ]]; then
                printf '%s\t%s\t%s\t%s\n' "$wt_path" "$branch" "${wt_path##*/}" "$is_bare"
            fi
...
_wt_get_repo_dir() {
    if [[ -n "$_WT_REPO_DIR" && -d "${_WT_REPO_DIR}/.git" ]]; then
        printf '%s' "$_WT_REPO_DIR"
        return 0
    fi

    if [[ -n "${WT_REPO_DIR:-}" && -e "$WT_REPO_DIR" ]]; then
        printf '%s' "$WT_REPO_DIR"
        return 0
    fi
...
_wt_check_update() {
    if ! _wt_get_auto_update_config; then
        return
    fi

    local repo_dir
    if ! repo_dir=$(_wt_get_repo_dir); then
        return
    fi
    ...
    if [[ -f "$cache_file" ]]; then
        ...
        cached_lines=("${(@f)$(<"$cache_file")}")

        local last_check="${cached_lines[1]:-}"
        if [[ "$last_check" =~ ^[0-9]+$ ]] && (( now - last_check < 86400 )); then
            local cached_version="${cached_lines[2]:-}"
            if [[ -n "$cached_version" ]] && (( $(_wt_compare_version "$_WT_VERSION" "$cached_version") < 0 )); then
                printf '%s\n' "$(_yellow "${_WT_WARNING} Update available: v${_WT_VERSION} → v${cached_version}. Run '${_wt_command_name} update' to update.")"
            fi
            return
        fi
    fi
    ...
    local remote_version
    if ! remote_version=$(_wt_get_remote_version "$repo_dir"); then
        print -r -- "$now" > "$cache_file"
        return
    fi
    ...
    {
        print -r -- "$now"
        print -r -- "$remote_version"
    } > "$cache_file"

    if (( $(_wt_compare_version "$_WT_VERSION" "$remote_version") < 0 )); then
        printf '%s\n' "$(_yellow "${_WT_WARNING} Update available: v${_WT_VERSION} → v${remote_version}. Run '${_wt_command_name} update' to update.")"
    fi
}
...
_wt_update() {
    ...
    if ! repo_dir=$(_wt_get_repo_dir); then
        ...
        printf '%s\n' "$(_yellow "   Set the WT_REPO_DIR environment variable or add 'repo_dir = /path/to/wt' to ~/.wtconfig")"
        return 1
    fi
    ...
    now=$(date +%s)
    {
        print -r -- "$now"
        print -r -- "$remote_version"
    } > "$HOME/.wt_update_check"

    local comparison
    comparison=$(_wt_compare_version "$_WT_VERSION" "$remote_version")
    ...
    read -r "response?Do you want to update? (y/N) "
    if [[ ! "$response" =~ ^[Yy]$ ]]; then
...
_wt_help() {
    ...
    echo
    printf '%s\n' "$(_cyan "Commands:")"
    printf '  %s\n                          %s\n' "$(_green "add <name> [type] [--from <worktree>]")" "Create a new worktree (type defaults to 'feature')"
...
_wt_init_main_worktree() {
    ...
    if [[ -n "$detected_branch" && -n "$worktrees" ]]; then
        ...
        while IFS=$'\t' read -r wt_path branch name is_bare; do
            [[ "$is_bare" == "true" ]] && continue
            if [[ "$branch" == "$detected_branch" ]]; then
                main_path="$wt_path"
    ...
    if [[ -z "$main_path" ]]; then
        ...
        # If default branch wasn't detected, fix refspec and retry
        # (handles bare repos cloned with git clone --bare instead of wt clone)
...
_wt_add() {
    ...
    local worktree_name="${positional_args[1]:-}"
    local branch_type="${positional_args[2]:-$_WT_DEFAULT_BRANCH_TYPE}"

    if [[ -z "$worktree_name" ]]; then
        printf '%s\n' "$(_red "${_WT_CROSS} Error: Please specify a name for the worktree")"
        printf '%s\n' "$(_yellow "   Usage: ${_wt_command_name} add <name> [type] [--from <worktree>]")"
        return 1
    fi
    ...
    if [[ -n "$from_worktree" ]]; then
        ...
        if ! $found_source; then
            ...
            printf '%s\n' "$(_yellow "   Run '${_wt_command_name} list' to see available worktrees")"
            return 1
        fi
        echo
    else
    ...
    read -r "response?Do you want to navigate to the new worktree? (Y/n) "
    if [[ ! "$response" =~ ^[Nn]$ ]]; then
        cd "$worktree_path"
    fi
}
...
_wt_remove() {
    ...
    if [[ -z "$name" ]]; then
        ...
        printf '%s\n' "$(_yellow "   Usage: ${_wt_command_name} remove <worktree-name>")"
        return 1
    fi
    ...
    local wt_path branch wt_name is_bare
    while IFS=$'\t' read -r wt_path branch wt_name is_bare; do
        [[ "$is_bare" == "true" ]] && continue
...
_wt_remove_all() {
    ...
    echo
    read -r "response?Are you sure you want to remove all worktrees? (y/N) "
    ...
    local i
    for i in {1..${#rm_paths[@]}}; do
        local p="${rm_paths[$i]}"
        local n="${rm_names[$i]}"
        local br="${rm_branches[$i]}"

        _wt_progress_start "Removing ${n}"
        git -C "$_wt_project_dir" worktree remove "$p" --force &>/dev/null

        if [[ $? -eq 0 ]]; then
            _wt_progress_complete "Removed ${n}" "success"

            # Delete the branch if it exists and is not the default
            if [[ -n "$br" && "$br" != "$default_branch" ]]; then
                git -C "$_wt_project_dir" branch -D "$br" &>/dev/null
                if [[ $? -eq 0 ]]; then
                    printf '  %s\n' "$(_green "${_WT_CHECK} Deleted branch '${br}'")"
                fi
            fi
        else
            _wt_progress_complete "Failed to remove ${n} (might have uncommitted changes)" "warning"
        fi
    done
...
_wt_clone() {
    ...
    if [[ -z "$repo_url" ]]; then
        ...
        printf '%s\n' "$(_yellow "   Usage: ${_wt_command_name} clone <url>")"
        return 1
    fi
    ...
    if [[ -z "$main_branch" ]]; then
        ...
        printf '%s\n' "$(_yellow "   You can manually create a worktree using: ${_wt_command_name} add <name>")"
        return 1
    fi
    _wt_progress_complete "Found default branch: ${main_branch}" "success"
...
wt() {
    ...
    # Handle update separately (doesn't require being inside a git repo)
    if [[ "$cmd" == "update" ]]; then
        _wt_update
        return
    fi

    # Handle version separately (doesn't require being inside a git repo)
...
if [[ -z "$_wt_command_name" && -f "${HOME}/.wtconfig" ]]; then
    while IFS= read -r _wt_cfg_line; do
        if [[ "$_wt_cfg_line" =~ ^[[:space:]]*command_name[[:space:]]*=[[:space:]]*(.+)[[:space:]]*$ ]]; then
            _wt_command_name="${match[1]}"
            break
        fi
    done < "${HOME}/.wtconfig"
fi
[[ -z "$_wt_command_name" ]] && _wt_command_name="wt"

# If a custom name is configured, create a wrapper function
